...
class Citizen:
    ...
    def get_citizen(self, cid: int) -> Optional[Citizen]:
        ...
        if self.cid == cid:
            ...
            for sub in self._subordinates:
                if isinstance(sub.get_citizen(cid), Citizen):
    ...
    def get_closest_common_superior(self, cid: int) -> Citizen:
        ...
        >>> c6 = Citizen(6, "Hookins National Lab", 3024, "Engineer", 50)
        >>> c7 = Citizen(7, "Hookins National Lab", 3025, "Engineer", 50)
        >>> c1.become_subordinate_to(c2)
        >>> c2.become_subordinate_to(c3)
        ...
        True
        >>> c7.get_closest_common_superior(1) == c1
        True
        >>> c6.get_closest_common_superior(1) == c1
        True
        >>> c7.get_closest_common_superior(6) == c6
        True
        """
        # Note: This method must call itself recursively

        if self.get_society_head() == self or \
                self.get_society_head().cid == cid:
            return self
        elif isinstance(self.get_citizen(cid), Citizen):
            return self
        else:
            head1 = self.get_superior()
            citizen2 = self.get_society_head().get_citizen(cid)
            head2 = citizen2.get_superior()
            if head1 == head2:
                return head1
            elif isinstance(citizen2.get_citizen(self.cid), Citizen):
                return citizen2
            else:
                return head1.get_closest_common_superior(head2.cid)
